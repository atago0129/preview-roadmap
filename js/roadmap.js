/**
 * original https://github.com/florentsolt/roadmap.js
 * オリジナルからの変更点
 * ・日付表示をローカライズ
 * ・色指定をローカライズ
...
(function() {
  ...
  var refresh = function(filter, topPadding, nodeIndex) {
    if (tasks[nodeIndex].length > 0) {
      ...
      if (filter) {
        ...
        node.innerHTML = "<a style='margin: 5px; margin-top: " + topPadding + "px; display: inline-block; float: right; border-radius: 3px; color: #fff; font-size: 12px; background: #999; padding: 6px 20px 6px 20px; text-decoration: none;' href='javascript:' onclick='Roadmap.refresh(null, null," + nodeIndex + ")'>&larr; 戻る</a>";
      }

      var grapType = node.getAttribute("data-graph-type");
      var options = {};
      if (grapType === "all" || grapType === "task") {
        options = draw(nodeIndex, node, tasks[nodeIndex].filter(function(task) {
      ...
      if (grapType === "all" || grapType === "people") {
  ...
  var parse = function() {
    // Tasks
    var colors = function colores_google(n) {
    ...
    d3.selectAll("div.roadmap").each(function() {
      ...
      for (var j = 0, line; line = lines[j], j < lines.length; j++) {
        ...
        if (!currentTask.name && !currentTask.group) {
          ...
          currentTask.color = currentTask.group === '' ? '#3182bd' : colors(currentTask.group.charCodeAt(0));
          continue;
        }
        ...
        if (line !== "") {
          ...
          people[nodeIndex].push({
            ...
            to: currentTask.to,
            name: !currentTask.group ? currentTask.name : currentTask.name + "(" + currentTask.group + ")",
            taskGroup: currentTask.group,
            color: currentTask.group === '' ? '#3182bd' : colors(currentTask.group.charCodeAt(0)),
  ...
  var draw = function(nodeIndex, node, items, options) {
    ...
    var axisText = svg.append("g")
      ...
      .append("text")
      .text(function(d){
        return d.name.replace(/^\[\d*\]/g, ''); // ソート用の接頭辞を削除
      })
      .attr("x", 10)
      .attr("y", function(d){
    ...
    var xAxisGroup = svg.append("g")
      ...
      .call(xAxis);
    var xAxisGroupBottom = svg.append("g")
    ...
    if (now > timeScale.domain()[0] && now < timeScale.domain()[1]) {
      ...
      xAxisGroupBottom.selectAll(".now")
        .attr("stroke", "red")
        .attr("opacity", 0.5)
    ...
    xAxisGroup.selectAll("text")
      ...
      .attr("font-size", 10)
      .attr("dy", "1em");
    xAxisGroupBottom.selectAll("text")
      .style("text-anchor", "middle")
      .attr("fill", "#000")
      .attr("stroke", "none")
      .attr("font-size", 10)
    ...
    xAxisGroup.selectAll(".tick line")
      .attr("stroke", "#dddddd")
      .attr("shape-rendering", "crispEdges");
    xAxisGroupBottom.selectAll(".tick line")
      .attr("stroke", "#dddddd")
      .attr("shape-rendering", "crispEdges");
    ...
    svg.on("mousemove", function () {
      ...
      if (xCoord > sidePadding) {
        ...
        verticalMouseText
          ...
          .text(d3.time.format("%-m月%-d日")(timeScale.invert(xCoord - sidePadding)))
          .style("display", "block");
      } else {
